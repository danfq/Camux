...
MODULE_DESCRIPTION("V4L2 loopback video device with Camux multi-reader support");
MODULE_AUTHOR("Vasily Levin, "
	      "IOhannes m zmoelnig <zmoelnig@iem.at>,"
	      "Stefan Diewald,"
	      "Anton Novikov"
	      "et al.");
MODULE_VERSION("0.15.4-camux1");
MODULE_INFO(camux_multi_reader, "1");
MODULE_LICENSE("GPL");
...
struct v4l2_loopback_device {
	...
	atomic_t open_count;
	/* Camux broadcasts one producer to independent capture clients.  Upstream
	 * models both sides as exclusive bit tokens; retain that for OUTPUT and
	 * timeout owners, while reference-counting shared CAPTURE ownership. */
	atomic_t capture_format_users;
	atomic_t capture_stream_users;
...
#define acquire_token(dev, opener, label, token)                          \
	acquire_token_ref(&(dev)->label##_tokens,                          \
			  &(opener)->label##_token,                         \
			  &(dev)->capture_##label##_users, (token))
#define release_token(dev, opener, label)                                 \
	release_token_ref(&(dev)->label##_tokens,                          \
			  &(opener)->label##_token,                         \
			  &(dev)->capture_##label##_users)
...
#define has_other_owners(opener, dev) \
	((~((dev)->format_tokens ^ (opener)->format_token) & V4L2L_TOKEN_MASK) || \
	 (has_capture_token((opener)->format_token) &&                         \
	  atomic_read(&(dev)->capture_format_users) > 1))
#define need_timeout_buffer(dev, token) \
	((dev)->timeout_jiffies > 0 || (token) & V4L2L_TOKEN_TIMEOUT)

static void acquire_token_ref(u32 *available, u32 *owned,
			      atomic_t *capture_users, u32 token)
{
	if (*owned == token)
		return;
	*owned = token;
	if (has_capture_token(token))
		atomic_inc(capture_users);
	*available &= ~token;
}

static void release_token_ref(u32 *available, u32 *owned,
			      atomic_t *capture_users)
{
	u32 token = *owned;

	if (!token)
		return;
	if (!has_capture_token(token) || atomic_dec_and_test(capture_users))
		*available |= token;
	*owned = 0;
}
...
static int vidioc_s_fmt_vid(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (!(dev->format_tokens & token) && !has_capture_token(token)) {
		result = -EBUSY;
			  ...
			  struct v4l2_requestbuffers *reqbuf)
{
	...
	switch (reqbuf->type) {
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		/* Capture buffers are a shared view over the producer's queue. */
		break;
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		if (!(dev->format_tokens & token ||
...
static void client_usage_queue_event(struct video_device *vdev)
{
	...
	((struct v4l2_event_client_usage *)&ev.u)->count =
		atomic_read(&dev->capture_stream_users);
...
static int start_fileio(struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	if ((!has_capture_token(token) && !(dev->stream_tokens & token)) ||
	    opener->io_method != V4L2L_IO_NONE)
		return -EBUSY;
...
static int v4l2_loopback_add(struct v4l2_loopback_config *conf, int *ret_nr)
{
	...
	atomic_set(&dev->capture_format_users, 0);
	atomic_set(&dev->capture_stream_users, 0);
