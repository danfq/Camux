...
import { CameraItem } from "./components/custom/data/camera";
import { WindowResizeHandles } from "./components/custom/layout/window-resize-handles";
import { WindowTitlebar } from "./components/custom/layout/window-titlebar";
import { SettingsDialog } from "./components/custom/dialogs/settings";
      ...
      <div className="grid h-full w-full grid-rows-[40px_minmax(0,1fr)] bg-background text-foreground">
        ...
        <main className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] gap-6 px-6 py-5">
          ...
          <Card className="min-h-0 gap-0 overflow-hidden py-0 shadow-sm" aria-labelledby="device-heading">
            ...
            <CardContent
              className="min-h-0 flex-1 overflow-auto p-0 [scrollbar-color:var(--border)_transparent]"
              aria-live="polite"
            >
              {loading && (
              ...
              {!loading && error && (
                <div className="flex h-full min-h-40 items-center justify-center gap-3 px-6 py-10" role="alert">
                  ...
                  <div>
                    ...
                    <p className="mt-1 max-w-md text-xs leading-relaxed text-muted-foreground wrap-break-word">
                      {error}
                    </p>
                  </div>
                </div>
